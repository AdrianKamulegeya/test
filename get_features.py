...
def get_manager_details(name):
    uri = "http://dbpedia.org/resource/" + name.replace(' ', "_").decode('utf-8')
    data = get_data(uri)
    manager_dict = get_dictionary(data, uri)
    clubs_managed = get_clubs_managed(manager_dict)

    manager_data = {'manager': name,
                    'uri': uri,
                    'clubs_managed': clubs_managed}

    with open('manager.pickle', 'wb') as handle:
        pickle.dump(manager_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
